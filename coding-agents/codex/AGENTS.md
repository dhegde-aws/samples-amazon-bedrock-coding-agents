...
`openai.gpt-5.5` through Bedrock. The AgentCore Runtime role supplies AWS SDK credentials, so
no model key is baked into the image. `AGENTS.md` is the project instruction convention,
