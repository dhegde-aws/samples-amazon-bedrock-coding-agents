...
# Make the sibling critter_lab importable when run from anywhere.
...
class InProcessClient:
    """Adapter over the reference critter_lab. No network, fully deterministic.
    ...
    def list_tools(self) -> list[dict[str, Any]]:
        return critter_lab.list_tools()

    def call_tool(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        return critter_lab.dispatch(name, arguments)
...
class RemoteMCPClient:
    ...
    Locally this points at the agents' reference server; on AgentCore it points at the
    Gateway. Gateway access is IAM-authenticated, so when ``MCP_SIGV4=1`` is set the
    request is SigV4-signed with botocore (service ``bedrock-agentcore``): same wire
    shape, signed transport. A JSON-RPC ``error`` raises ``MCPRemoteError``, which is
    exactly what the ``input_validation`` check expects for bad input.
    """
